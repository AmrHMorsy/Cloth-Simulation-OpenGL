...
Cloth::Cloth( glm::mat4 v, glm::mat4 proj, glm::vec3 c, glm::vec3 front, float fv, progressbar * b, glm::mat4 lightproj, glm::mat4 lightview ): num_progress_bars(10),light_position({0.484394f, 7.5f, 0.416144f}), light_color({100.0f, 100.0f, 100.0f}), view(v), projection(proj), camera(c), file_paths( {"../assets/Cloth/Albedo.png", "../assets/Cloth/AO.png", "../assets/Cloth/Normal.png", "../assets/Cloth/Roughness.png", "../assets/Cloth/Displacement.png", "../assets/Cloth/Metallic.png"} ), camera_front(front), fov(fv), number_sand_patches(0), bar(b), simulate(false), light_projection(lightproj), light_view(lightview)
{
    ...
    model = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f,2.2f,0.0f)) * glm::rotate(glm::mat4(1.0f), glm::radians(-90.0f),glm::vec3(0.0f,1.0f,0.0f)) * glm::scale( glm::mat4(1.0f), glm::vec3(1.0f,1.0f,1.0f) ) ;
    ...
    cloth_simulator = new Cloth_Simulator(parser->get_pure_vertices(), parser->get_faces(),parser->get_vertices(),fixed_points, 5.0f, 5.0f, 2.0f) ;
}

void Cloth::set( Skybox * skybox, Collision_Detector * collision_detector )
{
    this->skybox = skybox ;
    this->collision_detector = collision_detector ; 
    cloth_simulator->set(collision_detector) ; 
}
...
void Cloth::shade( Shadow * shadow )
{
    ...
    if( simulate ){
        ...
        std::vector<float> r = cloth_simulator->get_rendering_data() ;
        for( int i = 1 ; i < r.size() ; i+=8 ){
            if( r[i] <= -1.0f )
                r[i] = -1.0f ;
        }
        bind_vertices( r ) ;
...
std::vector<float> Cloth::get_mesh()
{
