...
Grass::Grass( glm::mat4 v, glm::mat4 proj, glm::vec3 c, glm::vec3 front, float fv, progressbar * b, glm::mat4 lightproj, glm::mat4 lightview ): num_progress_bars(10),light_position({0.484394f, 7.5f, 0.416144f}), light_color({100.0f, 100.0f, 100.0f}), view(v), projection(proj), camera(c), file_paths( {"../assets/Ground/Albedo.jpg", "../assets/Ground/AO.jpg", "../assets/Ground/Normal.jpg", "../assets/Ground/Roughness.jpg", "../assets/Ground/Displacement.jpg", "../assets/Ground/Metallic.jpg" } ), camera_front(front), fov(fv), number_sand_patches(0), bar(b), light_projection(lightproj), light_view(lightview)
{
...
void Grass::shade( unsigned int shadow_shader_program )
{
    ...
    glUniformMatrix4fv(glGetUniformLocation(shadow_shader_program, "model"), 1, GL_FALSE, &m[0][0]) ;
...
bool Grass::is_in_view( glm::vec3 pos )
{
...
std::vector<float> Grass::get_mesh()
{
