...
Bag::Bag( glm::mat4 v, glm::mat4 proj, glm::vec3 c, glm::vec3 front, float fv, progressbar * b ): num_progress_bars(10),light_position({0.484394f, 7.5f, 0.416144f}), light_color({100.0f, 100.0f, 100.0f}), view(v), projection(proj), camera(c), file_paths( {"../assets/Bag/Albedo.png", "../assets/Bag/AO.png", "../assets/Bag/Normal.png", "../assets/Bag/Roughness.png", "../assets/Bag/Displacement.png", "../assets/Bag/Metallic.png"} ), camera_front(front), fov(fv), number_sand_patches(0), bar(b)
{
    parser = new Parser(bar) ;
    parser->parse( "../assets/Bag/Bag.obj" ) ;
    shader = new Shader("../external/Shaders/Bag/Bag_vs.vs","../external/Shaders/Bag/Bag_fs.fs") ;
    generate_buffers() ;
    load_textures() ;
    shader_program = shader->get_shader_program() ;
    glUseProgram(shader_program) ;
    model = glm::translate(glm::mat4(1.0f), glm::vec3(-2.5f, 1.25f, 0.5f)) * glm::rotate(glm::mat4(1.0f), glm::radians(90.0f), glm::vec3(0.0f,1.0f,0.0f)) * glm::rotate(glm::mat4(1.0f), glm::radians(-5.0f), glm::vec3(0.0f,0.0f,1.0f))* glm::scale(glm::mat4(1.0f), glm::vec3(0.2f, 0.2f, 0.2f)) ;
    bind_vertices( parser->get_vertices() ) ;
    bind_indices( parser->get_vertices_faces() ) ;
    set_uniform_variables() ;
}

void Bag::set( Skybox * skybox )
{
    this->skybox = skybox ;
}

void Bag::generate_buffers()
{
...
void Bag::shade( unsigned int shadow_shader_program )
{
...
void Bag::bind_vertices( std::vector<float> vertices )
{
    glBindVertexArray(vao) ;
...
void Bag::update_time()
{
    float deltaTime = glfwGetTime() - time ;
    time += deltaTime ;
}

void Bag::bind_indices( std::vector<unsigned int> indices )
{
    glBindVertexArray(vao) ;
...
void Bag::set_uniform_variables()
{
...
void Bag::update_view( glm::vec3 camera, glm::mat4 v, glm::vec3 front )
{
    this->camera = camera ;
    camera_front = front ;
    glUseProgram(shader_program) ;
...
void Bag::update_progress_bar( int num_bars )
{
...
std::vector<float> Bag::get_vertices()
{
    return parser->get_vertices() ;  ; 
}

void Bag::generate_error( std::string msg )
{
