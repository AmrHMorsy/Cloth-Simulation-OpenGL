...
Blanket::Blanket( glm::mat4 v, glm::mat4 proj, glm::vec3 c, glm::vec3 front, float fv, progressbar * b, glm::mat4 lightproj, glm::mat4 lightview ): num_progress_bars(0),light_position({0.484394f, 7.5f, 0.416144f}), light_color({100.0f, 100.0f, 100.0f}), view(v), projection(proj), camera(c), file_paths( {"../assets/Blanket/Albedo.png", "../assets/Blanket/AO.png", "../assets/Blanket/Normal.png", "../assets/Blanket/Roughness.png", "../assets/Blanket/Displacement.png", "../assets/Blanket/Metallic.png"} ), camera_front(front), fov(fv), number_sand_patches(0), bar(b), simulate(false), light_projection(lightproj), light_view(lightview)
{
    parser = new Parser(bar) ;
    parser->parse( "../assets/Cloth/Cloth.obj" ) ;
    ...
    model = glm::translate(glm::mat4(1.0f), glm::vec3(-3.0f, 3.7f, 1.0f)) * glm::rotate(glm::mat4(1.0f), glm::radians(180.0f),glm::vec3(0.0f,1.0f,0.0f)) * glm::scale( glm::mat4(1.0f), glm::vec3(1.0f,1.0f,1.0f) ) ;
    ...
    fixed_points.push_back(992) ;
    std::vector<float> vert = parser->get_pure_vertices() ;
    cloth_simulator = new Cloth_Simulator(parser->get_pure_vertices(), parser->get_faces(),parser->get_vertices(),fixed_points, 5.0f, 0.0f,1.0f) ;
}

void Blanket::set( Skybox * skybox )
{
    this->skybox = skybox ;
}

void Blanket::generate_buffers()
{
...
void Blanket::shade( unsigned int shadow_shader_program )
{
...
void Blanket::shade( Shadow * shadow )
{
    glUseProgram(shader_program) ;
    if( simulate ){
...
void Blanket::bind_vertices( std::vector<float> vertices )
{
    glBindVertexArray(vao) ;
...
void Blanket::update_time()
{
    float deltaTime = glfwGetTime() - time ;
    time += deltaTime ;
}

void Blanket::bind_indices( std::vector<unsigned int> indices )
{
    glBindVertexArray(vao) ;
...
void Blanket::set_uniform_variables()
{
...
void Blanket::update_view( glm::vec3 camera, glm::mat4 v, glm::vec3 front )
{
    this->camera = camera ;
    camera_front = front ;
    glUseProgram(shader_program) ;
...
void Blanket::update_progress_bar( int num_bars )
{
...
void Blanket::start_simulation()
{
    simulate = true ;
}

void Blanket::activate_textures()
{
...
void Blanket::generate_error( std::string msg )
{
