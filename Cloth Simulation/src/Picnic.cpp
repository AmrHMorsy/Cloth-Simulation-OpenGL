...
Picnic::Picnic( glm::mat4 v, glm::mat4 proj, glm::vec3 c, glm::vec3 front, float fv, progressbar * b, glm::mat4 lightproj, glm::mat4 lightview ): num_progress_bars(0),light_position({0.484394f, 7.5f, 0.416144f}), light_color({100.0f, 100.0f, 100.0f}), view(v), projection(proj), camera(c), file_paths( {"../assets/Picnic/Albedo.png", "../assets/Picnic/AO.png", "../assets/Picnic/Normal.png", "../assets/Picnic/Roughness.png", "../assets/Picnic/Displacement.png", "../assets/Picnic/Metallic.png"} ), camera_front(front), fov(fv), number_sand_patches(0), bar(b), simulate(false), light_projection(lightproj), light_view(lightview)
{
    parser = new Parser(bar) ;
    parser->parse( "../assets/Picnic/Cloth.obj" ) ;
    ...
    model = glm::translate(glm::mat4(1.0f), glm::vec3(-3.0f, 3.7f, 3.0f)) * glm::rotate(glm::mat4(1.0f), glm::radians(180.0f),glm::vec3(0.0f,1.0f,0.0f)) * glm::scale( glm::mat4(1.0f), glm::vec3(1.0f,1.0f,1.0f) ) ;
...
void Picnic::set( Skybox * skybox )
{
    this->skybox = skybox ;
}

void Picnic::generate_buffers()
{
...
void Picnic::shade( unsigned int shadow_shader_program )
{
...
void Picnic::shade( Shadow * shadow )
{
    glUseProgram(shader_program) ;
    if( simulate ){
...
void Picnic::bind_vertices( std::vector<float> vertices )
{
    glBindVertexArray(vao) ;
...
void Picnic::update_time()
{
    float deltaTime = glfwGetTime() - time ;
    time += deltaTime ;
}

void Picnic::bind_indices( std::vector<unsigned int> indices )
{
    glBindVertexArray(vao) ;
...
void Picnic::set_uniform_variables()
{
...
void Picnic::update_view( glm::vec3 camera, glm::mat4 v, glm::vec3 front )
{
    this->camera = camera ;
    camera_front = front ;
    glUseProgram(shader_program) ;
...
void Picnic::update_progress_bar( int num_bars )
{
...
void Picnic::start_simulation()
{
    simulate = true ;
}

void Picnic::activate_textures()
{
...
void Picnic::generate_error( std::string msg )
{
