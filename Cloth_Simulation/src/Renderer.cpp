...
Renderer::Renderer( GLFWwindow * window_, progressbar * bar): fps(0), fps_time(0), window(window_)
{
    ...
    house = new Model( camera->get_view(), camera->get_projection(), camera->get_camera_position(), bar, {"../assets/House/Albedo.png", "../assets/House/AO.jpg", "../assets/House/Normal.png", "../assets/House/Roughness.png", "../assets/House/Displacement.png", "../assets/House/Metallic.png", "../assets/House/Opacity.png"}, "../assets/House/House.obj", light->get_projection(), light->get_view() ) ;
...
void Renderer::write_frame() 
{
    ...
    for(int y = 0; y < height; y++)
        std::copy_n(pixels.begin() + y * rowSize, rowSize, flippedPixels.begin() + (height - 1 - y) * rowSize);
    char filename[1024];
    frameNumber++ ;
    sprintf(filename, "frames/frame_%d.png", frameNumber);
    stbi_write_png(filename, width, height, 3, &flippedPixels[0], width * 3);
}
