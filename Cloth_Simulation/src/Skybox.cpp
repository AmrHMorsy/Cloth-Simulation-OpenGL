...
Skybox::Skybox( glm::mat4 view, glm::mat4 projection, glm::vec3 camera, progressbar * bar ): num_progress_bars(25)
{
    this->bar = bar ;
...
{
    int w, h, nc ;
    float * data = stbi_loadf("../assets/Sky/9.hdr", &w, &h, &nc, 0) ;
    if( !data )
...
//    view = view * glm::rotate(glm::mat4(1.0f), glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f)) ;
