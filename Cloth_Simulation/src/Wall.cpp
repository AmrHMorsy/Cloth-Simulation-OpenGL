...
Wall::Wall( glm::mat4 v, glm::mat4 proj, glm::vec3 c, glm::vec3 front, float fv, progressbar * b ): num_progress_bars(10),light_position({0.484394f, 7.5f, 0.416144f}), light_color({10.0f, 10.0f, 10.0f}), view(v), projection(proj), camera(c), file_paths( {"../assets/House/Wall/Albedo.png", "../assets/House/Wall/AO.png", "../assets/House/Wall/Normal.png", "../assets/House/Wall/Roughness.png", "../assets/House/Wall/Displacement.png", "../assets/House/Wall/Metallic.png"} ), camera_front(front), fov(fv), number_sand_patches(0), bar(b)
{
    t = glm::vec3(-1.0f, 0.0f, 0.0f) ;
    parser = new Parser(bar) ;
    parser->parse( "../assets/House/Wall/Wall.obj" ) ;
...
void Wall::set( Skybox * skybox )
{
    this->skybox = skybox ;
}

void Wall::generate_buffers()
{
...
void Wall::shade( unsigned int shadow_shader_program )
{
...
void Wall::bind_vertices( std::vector<float> vertices )
{
    glBindVertexArray(vao) ;
...
void Wall::update_time()
{
    float deltaTime = glfwGetTime() - time ;
    time += deltaTime ;
}

void Wall::bind_indices( std::vector<unsigned int> indices )
{
    glBindVertexArray(vao) ;
...
void Wall::set_uniform_variables()
{
...
bool Wall::is_in_view(glm::vec3 ocean_position )
{
    return true ;
...
void Wall::update_view( glm::vec3 camera, glm::mat4 v, glm::vec3 front )
{
    this->camera = camera ;
    camera_front = front ;
    glUseProgram(shader_program) ;
...
void Wall::update_progress_bar( int num_bars )
{
...
std::vector<float> Wall::get_vertices()
{
    return parser->get_vertices() ;  ;
}

void Wall::generate_error( std::string msg )
{
    std::cout << msg << std::endl ;
    exit(1) ;
}

void Wall::move(glm::vec3 g)
{
    t = t + g ;
