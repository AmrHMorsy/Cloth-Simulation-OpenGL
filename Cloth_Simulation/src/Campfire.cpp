...
Campfire::Campfire( glm::mat4 v, glm::mat4 proj, glm::vec3 c, glm::vec3 front, float fv, progressbar * b ): num_progress_bars(10),light_position({0.484394f, 7.5f, 0.416144f}), light_color({10.0f, 10.0f, 10.0f}), view(v), projection(proj), camera(c), file_paths( {"../assets/Campfire/Albedo.jpg", "../assets/Campfire/AO.jpg", "../assets/Campfire/Normal.jpg", "../assets/Campfire/Roughness.jpg", "../assets/Bag/Displacement.png", "../assets/Campfire/Metallic.jpg"} ), camera_front(front), fov(fv), number_sand_patches(0), bar(b)
{
    parser = new Parser(bar) ;
    parser->parse( "../assets/Campfire/Campfire.obj" ) ;
    shader = new Shader( "../external/Shaders/Campfire/Campfire_vs.vs", "../external/Shaders/Campfire/Campfire_fs.fs") ;
    generate_buffers() ;
    load_textures() ;
    shader_program = shader->get_shader_program() ;
    glUseProgram(shader_program) ;
    model = glm::translate(glm::mat4(1.0f), glm::vec3(-1.5f, 1.36f, 2.5f)) * glm::scale(glm::mat4(1.0f), glm::vec3(0.01f, 0.01f, 0.01f)) ;
    bind_vertices( parser->get_vertices() ) ;
    bind_indices( parser->get_vertices_faces() ) ;
    set_uniform_variables() ;
}

void Campfire::set( Skybox * skybox )
{
    this->skybox = skybox ;
}

void Campfire::generate_buffers()
{
...
void Campfire::shade( unsigned int shadow_shader_program )
{
...
void Campfire::shade( Shadow * shadow )
{
    glUseProgram(shader_program) ;
    update_time() ;
    activate_textures() ;
...
void Campfire::bind_vertices( std::vector<float> vertices )
{
    glBindVertexArray(vao) ;
...
void Campfire::update_time()
{
    float deltaTime = glfwGetTime() - time ;
    time += deltaTime ;
}

void Campfire::bind_indices( std::vector<unsigned int> indices )
{
    glBindVertexArray(vao) ;
...
void Campfire::set_uniform_variables()
{
...
void Campfire::update_view( glm::vec3 camera, glm::mat4 v, glm::vec3 front )
{
    this->camera = camera ;
    camera_front = front ;
    glUseProgram(shader_program) ;
...
void Campfire::update_progress_bar( int num_bars )
{
...
void Campfire::activate_textures()
{
...
std::vector<float> Campfire::get_vertices()
{
    return parser->get_vertices() ;  ;
}

void Campfire::generate_error( std::string msg )
{
